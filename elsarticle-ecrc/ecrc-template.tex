...
$b(R, T)$   & Mosquito birth rate (day$^{-1}$)   & $\dfrac{B_E  p_E(R)  p_L(R,T)  p_P(R)}{(\tau_E + \tau_L(T) + \tau_P)}$   \\ 
$a(T)$   & Biting rate (day$^{-1}$)  & $\dfrac{(T - T_1)}{D_1}$   \\ 
$\mu(T)$   & Mosquito mortality rate per capita (day$^{-1}$)   & $-\log(p(T))$  \\ 
$\tau_M(T)$   & Duration of sporozoite cycle (days)   & $\dfrac{DD}{(T - T_{min})}$   \\ 
$\tau_L(T)$   & Duration of larval development phase (days)   & $\dfrac{1}{c_1T + c_2}$  \\
$p(T)$   & Daily mosquito survival rate   & $e^{(-1 / (AT^2 + BT + C))}$ \\ 
$p_L(R)$   & Probability of larval survival dependent on rainfall   & $(\dfrac{4p_{ML}}{R_L^2})R(R_L - R)$  \\ 
...
$l({\tau_M})(T)$   &  Probability of mosquito survival during sporozoite cycle (day$^{-1}$)  & $p(T)^{\tau_M(T)}$  \\ 
